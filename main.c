...
int main(){
    ...
    simulation_1->debug_msgs = 0;   // Niveau de debug
    ...
    nest->Egg_list = realloc(nest->Egg_list, (++nest->Egg_count)*sizeof(Egg*));
    nest->Egg_list[nest->Egg_count-1] = queen;
    queen->Grow = 1;
    ...
    nest->Egg_list = realloc(nest->Egg_list, (++nest->Egg_count)*sizeof(Egg*));
    nest->Egg_list[nest->Egg_count-1] = worker;
    worker->Grow = 1;
